	...
	protected void doPost(HttpServletRequest req,HttpServletResponse res) throws IOException
	{
		...
		try (Connection con =DriverManager.getConnection("jdbc:mysql://localhost/newproject","root","As@13042000")){
			...
			String regNumb="";
				...
				CallableStatement cs2= (CallableStatement) con.prepareCall("{call storing_srcdest(?,?,?,?)}");
				cs2.setString(1, src);
				cs2.setString(2, dest);
				cs2.setString(3, date);
				cs2.setString(4, regNumb);
				
				ResultSet rset=cs2.executeQuery();
